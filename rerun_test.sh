    ...
    do
        ...
        if [ "$str" = "HEALTH_OK" ];then
            ...
            if [[ $size -gt 5 ]]; then
                ...
                # Executing COMMAND
                nohup run-ci -m "tier$TIER_NO" --ocs-version $OCS_VERSION --ocsci-conf=${OCS_UPI_DIR}/src/ocs-ci/conf/ocsci/production_powervs_upi.yaml --ocsci-conf ${OCS_UPI_DIR}/src/ocs-ci/conf/ocsci/lso_enable_rotational_disks.yaml --ocsci-conf ${OCS_CI_CONF} --cluster-name "ocstest" --cluster-path ${BASE_DIR}/ --collect-logs $TEST_CASE | tee ${RERUN_LOG_DIR}/$LOG_FILE_NAME.log 2>&1
